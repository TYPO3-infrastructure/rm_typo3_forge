class UserServicesController < ApplicationController
  ...
  def active
    ...
    @users = User.find_by_sql("SELECT  u.*, COUNT(i.id) AS issue_count FROM users u LEFT OUTER JOIN issues i ON i.author_id = u.id WHERE u.last_login_on > '#{3.months.ago.iso8601}' AND u.img_hash != '' GROUP BY u.id ORDER BY RAND() LIMIT 21")
