module MenuGeneratorHelper
  ...
  def render_header_image (project)
    ...
    if project then
       ...
       if FileTest.exist?("#{Rails.root}/public/headerimages/"+project.id.to_s+'.jpg') then
  ...
  def render_header_menu (project)
    ...
    # Display Subprojects
    Project.where('parent_id IS NULL').order(:sorting).each { |prj|
...
#    end
  ...
  def outputProjectsRecursively (project, rootline, level, inSelectTag = false)
    ...
    projects = Project.where([Project.visible_condition(User.current) + " AND parent_id = ?", project.id]).order("sorting, name")
